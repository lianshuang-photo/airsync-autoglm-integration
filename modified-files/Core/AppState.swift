...
class AppState: ObservableObject {
    ...
    init() {
        #if SELF_COMPILED
        // 自编译版本：强制启用会员功能，不读取 UserDefaults
        self.isPlus = true
        #else
    ...
    self.isMusicCardHidden = UserDefaults.standard.bool(forKey: "isMusicCardHidden")
        ...
        self.licenseDetails = LicenseDetails(
            key: "SELF-COMPILED-LICENSE",
            email: "self@compiled.local",
            productName: "AirSync Plus (Self Compiled)",
            orderNumber: 999999,
            purchaserID: "self-compiled",
            usesCount: 1,
            price: 0,
            currency: "USD",
            saleTimestamp: ISO8601DateFormatter().string(from: Date()),
...
//        postNativeNotification(id: "test_notification", appName: "AirSync Beta", title: "Hi there! (っ◕‿◕)っ", body: "Welcome to and thanks for testing out the app. Please don't forget to report issues to sameerasw.com@gmail.com or any other community you prefer. <3", appIcon: nil)
    ...
    // AI Assistant is always enabled
    ...
    // Toggle licensing
    #if SELF_COMPILED
    let licenseCheck: Bool = false  // 自编译版本：禁用许可证检查
    #else
